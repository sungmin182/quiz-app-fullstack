...
| 3000;
    ...
    // --- 미들웨어 설정 ---
    app.use(cors({
      origin: '[https://quiz-app-fullstack.onrender.com](https://quiz-app-fullstack.onrender.com)' // 프론트엔드 주소만 허용
    }));
    ...
    app.get('/api/questions', async (req, res) => {
        try {
            ...
            res.status(500).json({ error: '서버에서 문제 목록을 가져오는 데 실패했습니다.' });
        }
    });
    ...
    app.post('/api/questions', async (req, res) => {
        try {
            const { topic, gradeLevel, question, optionA, optionB, optionC, optionD, correctAnswer } = req.body;
    ...
    app.delete('/api/questions/:id', async (req, res) => {
        try {
            ...
            res.status(500).json({ error: '서버에서 문제를 삭제하는 데 실패했습니다.' });
        }
    });
